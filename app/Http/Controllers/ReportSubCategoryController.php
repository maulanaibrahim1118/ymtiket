...
class ReportSubCategoryController extends Controller
{
    public function index(Request $request)
    {
        ...
        foreach ($categories as $category) {
            foreach ($category->sub_category_tickets as $subCategory) {
                foreach ($agents as $agent) {
                    $ticketCount = $subCategory->ticket_details->where('agent_id', $agent->id)->where('status', 'resolved')->count();
                    $ticketCounts[$category->nama_kategori][$subCategory->nama_sub_kategori][$agent->id] = $ticketCount;
                }
                $ticketCounts[$category->nama_kategori][$subCategory->nama_sub_kategori]['totalTickets'] = $subCategory->ticket_details->where('status', 'resolved')->count();
            }
        }
